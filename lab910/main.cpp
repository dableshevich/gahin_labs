...
int countVowels(const string& s) {
    static const wstring vowels = L"аоуэеиюяёАОУЭЕИЮЯЁ";
...
Product* inputProducts(int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        products[i].vowels_count = countVowels(products[i].name);
    }
...
void outputProducts(Product* products, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cout << "Кол-во гласных: " << products[i].vowels_count << endl << endl; 
    }
}
